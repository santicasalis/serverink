...
const updateTattooArtist = async ({
  ...
  const tattooArtistFound = await TattooArtist.findByPk(id, {
    include: [{ model: TattooStyle, attributes: ["name"], required: false }],
  });
  if (tattooArtistFound) {
    const stylesNames = tattooArtistFound.TattooStyles.map(
      (style) => style.name
    );
    const updateData = {};
    ...
    if (tattooStyles.length) {
      const newStyles = tattooStyles.filter(
        (style) => !stylesNames.includes(style)
      );
      const removeStyles = stylesNames.filter(
        (style) => !tattooStyles.includes(style)
      );
      ...
      removeStyles.map(async (style) => {
        const removeStyle = allStyles.find(
          (eachStyle) => eachStyle.name == style
        );
